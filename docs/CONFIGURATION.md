...
The admin validator requires one address per VMID and one shared subnet for the
pool and gateway.
...
### PVE, NetBox, guest access, and retirement
...
NetBox is required in this release. The plugin creates or reuses one exact
cluster type, cluster, and managed tag, then refuses any record whose tag or
description does not match the ColdFront allocation identity.
...
stored on reservations. Cores, memory MiB, and disk GiB determine PVE/NetBox
configuration. Disable an unused flavor instead of renaming its code.
...
| `PVE_PROVISIONER_NETBOX_API_URL` | yes | NetBox base URL. |
| `PVE_PROVISIONER_NETBOX_TOKEN` | yes | Scoped NetBox API token. |
| `PVE_PROVISIONER_NETBOX_VERIFY_TLS` | yes | `True` or a CA bundle path. |
